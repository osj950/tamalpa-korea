...
type Props = { initial?: LibraryItem; mode: 'new' | 'edit' }
...
export default function LibraryForm({ initial, mode }: Props) {
  const [form, setForm] = useState({
    ...
    desc: initial?.desc ?? '',
    action: initial?.action ?? 'Download',
    fileInfo: initial?.fileInfo ?? '',
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const url = mode === 'new' ? '/api/admin/library' : `/api/admin/library/${initial!.id}`
    const method = mode === 'new' ? 'POST' : 'PUT'
    const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
    if (res.ok) { setStatus('success'); setTimeout(() => { window.location.href = '/admin/library' }, 800) }
  ...
  return (
    <div style={{ padding: '40px' }}>
      <div style={{ marginBottom: '32px' }}>
        ...
        <h1 style={{ fontSize: '28px', fontFamily: 'var(--font-cormorant), serif', fontWeight: 400, color: 'var(--dark)' }}>{mode === 'new' ? '새 자료 등록' : '자료 수정'}</h1>
      </div>

      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', marginBottom: '20px' }}>
          <Field label="유형 (태그)" required><input value={form.tag} onChange={e => set('tag', e.target.value)} placeholder="PDF / 영상 / 문서" required style={inputStyle} /></Field>
        ...
        <Field label="제목" required style={{ marginBottom: '20px' }}><input value={form.title} onChange={e => set('title', e.target.value)} placeholder="자료 제목" required style={inputStyle} /></Field>
        ...
        <Field label="파일 정보" style={{ marginBottom: '20px' }}><input value={form.fileInfo} onChange={e => set('fileInfo', e.target.value)} placeholder="예: PDF · 한국어 · 16페이지 · 2.3MB" style={inputStyle} /></Field>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          ...
          <a href="/admin/library" style={{ fontSize: '14px', color: 'var(--text-light)', textDecoration: 'none' }}>취소</a>
        </div>
      </form>
    </div>
  )
