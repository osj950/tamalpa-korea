    ...
    <div style={{ padding: '40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          ...
          <h1 style={{ fontSize: '28px', fontFamily: 'var(--font-cormorant), serif', fontWeight: 400, color: 'var(--dark)' }}>소식 관리</h1>
        </div>
        <a href="/admin/news/new" style={{ background: 'var(--gold)', color: 'white', padding: '10px 24px', textDecoration: 'none', fontSize: '13px', letterSpacing: '0.06em', fontFamily: 'var(--font-cormorant), serif' }}>
          + 새 소식
        </a>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: 'white', border: '1px solid rgba(158,138,107,0.2)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={4} style={{ padding: '32px 20px', textAlign: 'center', color: 'var(--text-light)', fontSize: '14px' }}>소식이 없습니다.</td></tr>
              ) : items.map(item => (
                <tr key={item.id} style={{ borderBottom: '1px solid rgba(158,138,107,0.1)' }}>
                  ...
                  <td style={{ padding: '14px 20px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <a href={`/admin/news/${item.id}`} style={{ fontSize: '13px', color: 'var(--gold)', textDecoration: 'none', marginRight: '16px' }}>수정</a>
