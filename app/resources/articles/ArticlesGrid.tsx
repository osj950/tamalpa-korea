      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '28px' }}>
          {articles.map(item => (
            <a
              key={item.id}
              href={`/resources/articles/${item.id}`}
              style={{
            ...
            >
              ...
              <span style={{ fontFamily: 'var(--font-cormorant), serif', fontSize: '13px', color: 'var(--gold)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Read More →</span>
            </a>
          ))}
        </div>
      </div>
